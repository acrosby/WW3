...
PROGRAM W3BOUNC
  ...
  !
  INTEGER                 :: IX, IY, ISEA, I,JJ,IP,IP1,IP2,J,IT,    &
...
# rather than reading a list of files (ww3_bounc)
  ...
  IF ( INXOUT.EQ.'WRITE') THEN
    ...
    ALLOCATE(dimid(NBO2,3+1),dimln(NBO2,3+1),ncid(NBO2))
    if (NBO2 .ne. 1) then
        STOP 'ww3_bounc1 expects 1 input file - use ww3_bounc for 1 point per file'
    endif
    ! Determine number of points in file
    WRITE(6,*) 'Checking file:',trim(SPECFILES(1))
    ...
    call check_err(iret)
    ! NBO2 now reflects the number of points in the single netCDF file
    ...
    DO IP=1,1
      ...
      IF ( FLAGLL ) THEN
        ...
        CALL CHECK_ERR(IRET)
        IRET=NF90_GET_VAR(NCID(IP), VARID(2), LATS, start=(/1,1/), count=(/NBO2,1/))
        CALL CHECK_ERR(IRET)
        IRET=NF90_INQ_VARID(NCID(IP), 'longitude', VARID(3))
        CALL CHECK_ERR(IRET)
        IRET=NF90_GET_VAR(NCID(IP), VARID(3), LONS, start=(/1,1/), count=(/NBO2,1/))
        CALL CHECK_ERR(IRET)
      ELSE
      ...
      iret = nf90_get_var(ncid(IP), varid(6), station(:,:),      &
                              start=(/1,1/),count=(/16, NBO2/))
      call check_err(iret)
      WRITE(6,*) 'STATIONS', station
      
      do IP2 = 1,NBO2
      iret=nf90_get_var(ncid(IP),varid(7),SPEC2D(:,:,:,IP2),       &
                        start=(/1,1,IP2,1/),count=(/NTHI,NKI,1,NTI/))
      end do 
